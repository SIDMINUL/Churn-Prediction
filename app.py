...
with st.expander("About this model"):
    st.write(
        "The app trains a Logistic Regression model on the complete customer dataset at startup. "
        "Numeric features are imputed and standardized, while categorical features are one-hot encoded. "
        "This avoids treating categories such as contract type or internet service as arbitrary numbers."
    )
            ...
            if col in categorical_features:
                ...
                if minimum == maximum:
                    ...
                    input_data[col] = st.number_input(
                        col, min_value=minimum, max_value=maximum, value=default, key=f"input_{col}"
                    )
...
if submitted:
    ...
    with result_col:
        if churn:
            ...
            st.write("The model estimates a higher churn probability. Consider targeted retention offers or proactive support.")
        else:
            st.success("✅ Customer is likely to STAY")
            st.write("The model estimates a lower churn probability for this customer.")
    with score_col:
